...
/* La donnee auto-mpg est ici scindee en deux tables partageant identifiant,
   pour reproduire l'assemblage (tri + fusion + concatenation) du projet. */
data autompg1;
  set projet_mpg (keep=identifiant mpg cylindres origine annee_du_modele);
run;
data autompg2;
  set projet_mpg (keep=identifiant deplacement puissance poids acceleration);
run;

/* Concatenation autompg1 et autompg2 */
proc sort data=autompg1; by identifiant; run;
proc sort data=autompg2; by identifiant; run;

data autompg12;
  merge autompg1 autompg2;
  by identifiant;
run;
